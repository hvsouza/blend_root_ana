...
void read_warm(){
  TFile *f = new TFile("warm_data.root","RECREATE");
  TTree *t1 = new TTree("t1","t1");
  
  Double_t vnom,vin,vout,vinstd,voutstd,areain,areainstd,areaout,areaoutstd,offset,offsetstd,dataset,basein,baseinstd,baseout,baseoutstd;
  ...
  ifstream fin; 
  fin.open("20220120_sweeping_1000_warm.txt");
  ...
  while(!fin.fail()){
    ...
    fin >> vnom >> vin >> vinstd >> areain >> areainstd >> vout >> voutstd >> areaout >> areaoutstd >> offset >> offsetstd >>  baseout >> baseoutstd >> dataset;

    if(vnom == 0){
      basein = areain;
      baseinstd = areainstd;
      areaout = baseout;
      areaoutstd = baseoutstd;
    }
    if(fin.bad() || fin.fail()){
      break;
    }
