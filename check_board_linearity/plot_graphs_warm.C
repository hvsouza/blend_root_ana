void plot_graphs_warm(){
  
  TFile *f = new TFile("warm_data.root","READ");
  TTree *t1 = (TTree*)f->Get("t1");
  
  Int_t n = t1->GetEntries();
  
  Double_t vnom[n],vin[n],vout[n],vinstd[n],voutstd[n],areain[n],areainstd[n],areaout[n],areaoutstd[n],offset[n],offsetstd[n],dataset[n],basein[n],baseinstd[n],baseout[n],baseoutstd[n],baselineoffset[n];
  Double_t r_vnom,r_vin,r_vout,r_vinstd,r_voutstd,r_areain,r_areainstd,r_areaout,r_areaoutstd,r_offset,r_offsetstd,r_dataset,r_basein, r_baseinstd,r_baseout,r_baseoutstd,r_baselineoffset;
  ...
  for(Int_t i = 0; i<n; i++){
    ...
    vinstd[i] = r_vinstd; 
    voutstd[i] = r_voutstd; 
    areain[i] = r_areain - r_basein; 
    areainstd[i] = sqrt(pow(r_areainstd,2)+pow(r_baseinstd,2)); 
    areaout[i] = r_areaout-r_baseout; 
    areaoutstd[i] = sqrt(pow(r_areaoutstd,2)+pow(r_baseoutstd,2)); ; 
    offset[i] = r_offset; 
    ...
    correction_factor = r_offset/r_baselineoffset;
    correction_factorstd = r_offsetstd/r_baselineoffset;

    corrected_areaout[i] = r_areaout-correction_factor*r_baseout;
    corrected_areaoutstd[i] = sqrt(pow(r_areaoutstd,2)+pow(r_baseoutstd*correction_factor,2)+ pow(r_baseout*correction_factorstd,2)); ; 
  }
  ...
  g2->GetXaxis()->SetTitle("Amplitude input (mV)");
