...
void getNoise(){
  ...
  ANALYZER zn("zn");
  zn.setAnalyzer("./sphe_waveforms_Ch3.root");
...
void getSPEFFT(){
  ...
  ANALYZER zpe("zpe");
  zpe.setAnalyzer("./sphe_waveforms_Ch3.root");
  // TGraph *gsphe = (TGraph*)zpe.f->Get("mean");
  // zpe.getWaveFromGraph(gsphe);
  //
  vector<Double_t> avgspe(zpe.n_points);
  Int_t navg = 0;
  zpe.getSelection("Ch3.selection==1");
  cout << "Got selected by fit" << endl;
  zpe.selectByAmplitude(16, 0, 5000, 36, "higher"); // remove pretrigger with pulse
