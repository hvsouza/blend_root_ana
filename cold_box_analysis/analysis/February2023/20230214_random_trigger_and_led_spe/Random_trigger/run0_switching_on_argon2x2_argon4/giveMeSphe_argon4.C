...
void giveMeSphe_argon4(){
    ...
    dark.tolerance    = 2.6;      // n sigmas (smoothed) (not used for led)
    ...
    dark.timeLow         = 120; // integration time before peak (not used for led)
    dark.timeHigh        = 260; // integration time after peak (not used for led)
    dark.lowerThreshold  = -1; // threshold to detect noise (normal waveform) (not used for led)
    dark.maxHits         = 10;  // maximum hit before discarding   (not used for led)
    dark.too_big         = 60; // if there is a peak > "too_big" .. wait "waiting" ns for next peak
    dark.waiting         = 1000;
    ...
    dark.diff_multiplier = 300;  //derivative can be very small. Use this to make it easier to see
    dark.derivate_raw    = false; // If you apply a denoise in the data and want to take derivative on that, set to false
    dark.withfilter      = true;

    dark.dtime = 4.;            // time step in ns


    dark.get_wave_form           = true; // for getting spe waveforms
    dark.mean_before             = 1000; // time recorded before and after the peak found
    dark.mean_after              = 1000;
    dark.sphe_charge             = 1.70946e2; //wave0
