...
void giveMeSphe(){
    ...
    dark.timeLow         = 104;   // integration time before peak (not used for led)
    dark.timeHigh        = 100; // integration time after peak (not used for led)
    dark.lowerThreshold  = -7;  // threshold to detect noise (normal waveform) (not used for led)
    dark.maxHits         = 5;   // maximum hit before discarding   (not used for led)
    dark.too_big         = 300; // if there is a peak > "too_big" .. wait "waiting" ns for next peak
    dark.waiting         = 4000;
    ...
    dark.sphe_charge = 2230.98; //wave0
    dark.sphe_charge2 = 5371.42; // wave0
    dark.sphe_std      = 800.36;
