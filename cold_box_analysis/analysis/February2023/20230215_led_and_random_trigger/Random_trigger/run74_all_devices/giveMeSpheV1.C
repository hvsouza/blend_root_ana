...
void giveMeSpheV1(){
    ...
    dark.tolerance    = 2.2;      // n sigmas (smoothed) (not used for led)
    ...
    dark.timeHigh        = 180; // integration time after peak (not used for led)
    dark.lowerThreshold  = -3; // threshold to detect noise (normal waveform) (not used for led)
    dark.maxHits         = 5;  // maximum hit before discarding   (not used for led)
    dark.too_big         = 80; // if there is a peak > "too_big" .. wait "waiting" ns for next peak
    dark.waiting         = 2000;
    dark.filter          = 8;  // one dimentional denoise filter (0 equal no filder)
    dark.interactions    = 14;  // for moving avarage filter (not used on led)
    dark.ninter          = 2;   // N times that moving average is applied
    dark.diff_multiplier = 40;  //derivative can be very small. Use this to make it easier to see
    ...
    dark.sphe_charge             = 1.28301e3; //wave0
    dark.sphe_charge2            = 5371.42; // wave0
    dark.sphe_std                = 2.73942e2;
    ...
    dark.social_distance = 0.5;   // demands that there is a minimum distance of social_distance * timeHigh between 2 consecutive peaks found
