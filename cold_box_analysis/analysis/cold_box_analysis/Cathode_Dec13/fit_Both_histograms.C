...
vector<OUTPUT> out(n);
vector<OUTPUT> out2(n);
vector<TBranch*> br(n);
vector<TBranch*> br2(n);
...
Double_t fit_start = 200; // in charge
...
void makeSpectrum(Double_t &val, Double_t &val2, Double_t par[]){
  Double_t res;
  Double_t res2;
...
void fcn(Int_t &npar, Double_t *gin, Double_t &f, Double_t *par, Int_t iflag)
{
    ...
    if(type == "charge") makeSpectrum(out[0].charge,out2[0].charge,par);
    else makeSpectrum(out[0].slow,out2[0].slow,par);

    for(Int_t i=0; i<n_bins; i++) {
      y[i] = (hfit->GetBinContent(i+1));
      delta  = (y[i]-z[i]);
      // cout << y[i] << " " << z[i] <<  " " << i << " " << fit_start_bin << endl;
      if(z[i]>0 && i+1>=fit_start_bin && i < fit_finish_bin)chisq += delta*delta/z[i];
    }
    f = chisq;
    fchisq.push_back(f);
...
void fit_Both_histograms(){
  Int_t n_bins_charge = 100;
  ...
  f = new TFile("C1_output_charges.root","READ");
  f2 = new TFile("C2_output_charges.root","READ");
  t1 = (TTree*)f->Get("t1");
  t2 = (TTree*)f2->Get("t1");
  for (Int_t i = 0; i<n; i++){
    br[i] = t1->GetBranch(Form("E%.0f",volts[i]));
    br[i]->SetAddress(&out[i]);
    br2[i] = t2->GetBranch(Form("E%.0f",volts[i]));
    br2[i]->SetAddress(&out2[i]);
    ...
    for(Int_t j = 0; j<t1->GetEntries(); j++){
      ...
      br2[i]->GetEvent(j);
      if(type=="charge"){
        hcharge[i]->Fill(out[i].charge/sphe_charge);
        hcharge[i]->Fill(out2[i].charge/sphe_charge);
      }
      else{
        hcharge[i]->Fill(out[i].slow/sphe_charge);
        hcharge[i]->Fill(out2[i].slow/sphe_charge);
      }
    }
  }
  ...
  if(type=="charge") makeSpectrum(out[0].charge,out2[0].charge,param);
  else makeSpectrum(out[0].slow,out2[0].slow,param);
  ...
  hfitCharge->GetXaxis()->SetTitle("Photo-electrons");
    // hcharge[0]->GetXaxis()->SetTitle("Photo-electrons from slow comp.");
  hfitCharge->GetYaxis()->SetTitle("# of events");
  
  hfitCharge->Draw("hist");
  hcharge[1]->Draw("hist same");
  hcharge[1]->SetLineWidth(2);
  hcharge[1]->SetLineColor(kRed);
