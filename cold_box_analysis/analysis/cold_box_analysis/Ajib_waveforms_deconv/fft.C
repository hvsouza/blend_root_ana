...
void shift_waveform(vector<Double_t> &y_raw){
  ...
  Int_t old_peak_start = 4000/4; // the deconvolved waveform peak starts more or less here.
    Int_t new_peak_start = 800/4; // I will make it start at 800 ns
 ...
 void fft()
 {
   ...
   TH1D *hsignal = (TH1D*)fwvf->Get("total_waveform");
   TH1D *hsphe = (TH1D*)fsphe->Get("summed_wf");
...
//    func_fit2->SetParameter(8,0);
   ...
   Double_t cutoff_frequency = 40; // In MHz
