...
void create_histograms_argon4(){
  gStyle->SetCanvasPreferGL(kFALSE);
  string mychannel = "Ch3";
  string device = "miniArapuca 47V";
  if(mychannel == "Ch2") device = "miniArapuca 37";
...
//     h[i]->GetXaxis()->SetRangeUser(-0.04,0.04);
    string hist_name = Form("%s - LED %.2f V",device.c_str(),volts[i]);
...
//   gSTDs->Draw("AP []");
  ...
  TF1 *myfunc3 = new TF1("myfunc3","pol1",-20,2100);
  gpeaks->Fit("myfunc3");
