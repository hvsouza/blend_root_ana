...
vector<string> infos = {"1kHz oscillation","Everything turned on","SiPMs off","SiPMs and Analog Converters off","Everything off","Everything on again"};
vector<string> infos_ffts = {"Everything turned on","SiPMs off","SiPMs and Analog Converters off","SiPMs and Analog Converters off 2","Everything off","Everything on again"};

vector<Color_t> mycolors = {kBlue,kGreen+2,kBlack,kBlack,kRed,kPink}; 
vector<Color_t> mycolors_wvf= {kBlue,kBlue, kGreen+2,kBlack,kRed,kPink}; 
...
void plot_graphs(){
  ...
  for(Int_t i = 0; i<n; i++){
    fwvf[i].open(Form("C1--100V-2000V--0000%d.txt",i),ios::in);
    ffft[i].open(Form("SpecAn--100V-2000V--0000%d.txt",i),ios::in);
    ...
    gfft[i]->SetNameTitle(Form("fft - %s",infos_ffts[i].c_str()),Form("fft - %s",infos_ffts[i].c_str()));
    setWvfGraph(gwvf[i]);
    setFFTGraph(gfft[i]);
    gwvf[i]->SetLineColor(mycolors_wvf[i]);
    gfft[i]->SetLineColor(mycolors[i]);
    gfft[i]->GetYaxis()->SetRangeUser(-79,-53);
