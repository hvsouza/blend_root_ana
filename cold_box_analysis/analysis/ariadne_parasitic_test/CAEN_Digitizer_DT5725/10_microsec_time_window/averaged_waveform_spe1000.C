...
void averaged_waveform_spe1000(){
  ...
  TFile *fout = new TFile("few_pe_response1000.root","RECREATE");
  ...
  for(Int_t k = 0; k<memorydepth; k++){
    ...
    if(k<1000)hpe->SetBinContent(k+1,avg[k]/total_waveforms);
    else hpe->SetBinContent(k+1,0);
  }
