...
//     h[i]->GetXaxis()->SetRangeUser(-0.04,0.04);
    string hist_name = Form("C2MiniXArapuca A4ch1 - LED %.1f V",volts[i]);
    ...
    cpeak->cd(i+1);
    hpeak[i]->Draw();
    string histp_name = Form("C2MiniXArapuca A4ch1 Peak - LED %.1f V",volts[i]);
