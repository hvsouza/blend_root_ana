...
void create_histograms_channels_sum(){
  string mychannel = "Ch1";
  string mychannel2 = "Ch2";
  ...
  vector<ADC_DATA> ch2(n);
  vector<TBranch *> bch(n);
  vector<TBranch *> bch2(n);
  vector<TH1D*> h(n);
  vector<TH1D*> hpeak(n);
  TH2D *h2p = new TH2D("h2p","h2p",800,-50,750,1000,0,1.);
  TH2D *h2 = new TH2D("h2","h2",300,0,30,4000,-50,750);
  ...
  for(Int_t i = 0; i<n; i++){
    ...
    bch2[i] = t1[i]->GetBranch(mychannel2.c_str());
    bch[i]->SetAddress(&ch[i]);
    bch2[i]->SetAddress(&ch2[i]);
    ...
    // for(Int_t j = 0; j<2000; j++){
      ...
      bch2[i]->GetEvent(j);
      charge = 0;
      if(ch[i].selection==0 && ch2[i].selection==0){
...
//   gSTDs->Draw("AP []");
  ...
  TF1 *myfunc2 = new TF1("myfunc2","pol1",0.5,700);
