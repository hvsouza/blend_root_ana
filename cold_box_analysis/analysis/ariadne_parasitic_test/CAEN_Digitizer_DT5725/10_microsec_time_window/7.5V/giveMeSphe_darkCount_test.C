...
void giveMeSphe_darkCount_test(){
    ...
    dark.tolerance = 2.7; // n sigmas (smoothed)
    dark.baseLimit = 3; // higher then this wont contribute to the baseline abs(baseLimit)
    dark.baselineTime = 5000;
    dark.start = 3980; // start the search for peaks or start the integration (led)
    dark.finish = 4120; // fisish the search or finish the integration (led)
    
    dark.timeLimit = 0; // time after LED signal
    dark.timeLow = 0; // integration time before peak
    dark.timeHigh = 176; // integration time after peak
