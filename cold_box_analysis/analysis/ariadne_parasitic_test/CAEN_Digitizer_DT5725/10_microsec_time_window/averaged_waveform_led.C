...
void averaged_waveform_led(){
  ...
  TFile *fout = new TFile("led_response.root","RECREATE");
  TH1D *hled = new TH1D("hled","hled",memorydepth,0,4*memorydepth);
  ...
  for(Int_t k = 0; k<memorydepth; k++){
    hled->SetBinContent(k+1,avg[k]/total_waveforms);
  }



  fout->WriteTObject(hled,"averaged_led","TObject::kOverwrite");
