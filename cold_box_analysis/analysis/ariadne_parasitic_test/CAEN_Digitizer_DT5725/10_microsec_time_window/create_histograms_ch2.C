...
//     h[i]->GetXaxis()->SetRangeUser(-0.04,0.04);
    string hist_name = Form("C1XArapuca A4ch2 - LED %.1f V",volts[i]);
    ...
    cpeak->cd(i+1);
    hpeak[i]->Draw();
    string histp_name = Form("C1XArapuca A4ch2 Peak - LED %.1f V",volts[i]);
...
//   gSTDs->Draw("AP []");
  ...
  TF1 *myfunc2 = new TF1("myfunc2","pol1",0.5,350);
