...
void Simp3_updatedanalysis(){
  ...
  TFile *f=new TFile("july13_led4p7_ch3_36V_2.root","RECREATE");
  
  TH1D *Total_charge=new TH1D("Total_charge","Integrated charge;Charge [V*ns];entries",2000,-2.0,8.0);
  TH2D *time_vs_signal=new TH2D("time_vs_signal",";time [ticks];signal[V]",2000,0,2000,1000,-0.5,0.5);
  TH1D *amplitude =new TH1D("amplitude","",1000,-0.5,0.5);
  ...
  TGraph *gr[1000];
  TGraph *gr_denoised[1000];
  double times[2000]={0.0};
  double waveform[2000]={0.0};
  int entry=0;
  std::vector<float> timev(2000);
  ...
  ifstream file("july13_led4p7_ch3_36V_2.txt");
  int line=0;
  ...
  while (!file.eof()){
	...
	waveform[i]=y;
      ...
      for(Int_t n=0; n<2000 ; n++){if(n>=mobileAVG && n<2000-mobileAVG){
	  ...
	  else if(n>=2000-mobileAVG){
	    for(Int_t i1=n-mobileAVG; i1<2000; i1++){avg=avg+waveform[i1]; c2=c2+1;}
	    avg=avg/c2; ondama[n]=avg; avg=0; c2=0;}}}
            
      for(Int_t i1=0; i1<2000; i1++){input[i1]=ondama[i1]; output[i1]=input[i1];}
            
      TV1D_denoise(input,output,2000,0.005);
      for(Int_t i1=0; i1<2000; i1++){
	...
	ondaden[i]=ondaden[i]-base;
      ...
      if(entry<1000){
	gr[entry]=new TGraph(2000,times,waveform);
	...
	gr_denoised[entry]->Write(Form("gr_denoised_%d",entry));
      ...
      //=================== Integral ====================================================
      sum=0.; sum2=-2000.; sigma2=-10;ampl=-100;index=0;
      for(int i=0;i<2000;i++){
	if(i>1000 && i<1100){//noise
	  sum+=ondaden[i];
