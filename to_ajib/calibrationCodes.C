...
//     vector<TGraph*> gwaveforms(npeaks);
    ...
    for(Int_t i = 0; i<npeaks; i++){
      ...
      if((peakPosition[i]+5*timeHigh>=peakPosition[i+1] && i+1!=npeaks) || next_is_bad || peakPosition[i]<waitingInterval){
            ...
            if(get_wave_form){
              ...
              if(notAGoodWaveform[i]==false){
                if(statcharge[i]>=delta/1.2 && statcharge[i]<=delta*1.4){
                  naverages++;
                  valid = true;
                  for(Int_t j = low_cut/4; j<(mean_after+mean_before)/4; j++){
...
void mean_signal(string filename){
  ...
  for(Int_t k = 0; k<channels.size();k++) hpersistence[k] = new TH2D(Form("hpersistence_%i",channels[k]),Form("hpersistence_%d",channels[k]),5000,0,20000,500,-0.001,0.002);
    ...
    for(Int_t k = 0; k<channels.size();k++){
      ...
      if(ch[k].charge>minval && ch[k].charge<maxval && ch[k].peak<avoid_saturation){
        if(checkFprompt(ch[k].fprompt,fprompt,mustbe)){
          norm[k]+=1;
          for(Int_t j = 0; j<memorydepth; j++){
            hpersistence[k]->Fill(j*dtime,ch[k].wvf[j]/ch[k].charge);
            avg[k][j]+=ch[k].wvf[j]/ch[k].charge;
          }
        }
      }
    }
